<?php
   ...
   $coumnName='code,type,title,advise_content,reply_content,finish_time,okreply_status';//session,time,
   $hw='';//0:5%,1:5%,2:5%,3:5%,4:10%,5:30%,6:5%,6:20%,6:5%';//每列的宽度
   $index=0;//是否显示序号 0 不显示  1 显示//html_entity_decode
   $idName='id';//关键字的属性名称//
   //cmd=内部名称：动作：图片：标题
   $cmd='详细信息:Advise/previewReply::详细信息';//,题目:PisaExamsData/index::题目';//操作的命令
...
<script>
    function previewReply(id) {
        $.dialog.open('<?php echo $this->createUrl("Advise/previewReply"); ?>&id=' + id, {
            id: 'order',
            lock: true,
            opacity: 0.3,
            title: '预览',
